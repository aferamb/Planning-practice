...
# Example:
# python3 generate-problem.py -d 2 -r 2 -l 4 -p 4 -c 6 -g 4 -a 4
...
def setup_content_types(options):
    while True:
        ...
        max_goals = sum(min(num_crates, options.persons) for num_crates in num_crates_with_contents)
...
def setup_person_needs(options, crates_with_contents):
    ...
    for _ in range(options.goals):
        ...
        while not generated:
            ...
            if (
                goals_per_contents[rand_content] < len(crates_with_contents[rand_content])
...
def main():
    ...
    domain_name = "ubermedics-carriers-temporal"
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        for i in range(len(person)):
            ...
            f.write(f"\t(person-free {person[i]})\n")
